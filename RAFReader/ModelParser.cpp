...
std::shared_ptr<Skeleton> ModelParser::ParseSkeleton(std::string filePath)
{
	std::ifstream file(filePath.c_str(), std::ios::binary);
	file.seekg(0, file.end);
	int length = file.tellg();
	...
	char *memory = new char[length];
	file.read(memory, length);
	return ParseSkeleton(memory);
}

std::shared_ptr<Skeleton> ModelParser::ParseSkeleton(const char* memory)
{
	int position = 0;
	...
	return std::shared_ptr<Skeleton>(new Skeleton(bones, boneIDs));
}
